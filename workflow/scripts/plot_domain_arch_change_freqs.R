...
main <- function() {
  ...
  domain_arch_changes <- read_csv('../../results/aligned_ortholog_domain_architectures.csv') %>%
    ...
    mutate(species_is_outgroup = ifelse(species != 'R_allo', species_is_outgroup,
                                        TRUE),
           ref_ortholog_essential = ref_ortholog %in% essential_ref_genes,
...
plot_DA_change_rates <- function(domain_arch_changes) {
  n_microsp <- nrow(filter(domain_arch_changes, species_is_outgroup == 'Microsporidia'))
  n_outgroup <- nrow(filter(domain_arch_changes, species_is_outgroup == 'Outgroup'))
  
  plot_df <- domain_arch_changes %>%
    rename(group = species_is_outgroup) %>%
    group_by(group, ref_ortholog_essential, DA_conservation) %>%
    summarise(n = n())
  
  ggplot(data = plot_df, aes(x = DA_conservation, y = n, fill = ref_ortholog_essential)) +
    geom_bar(stat = 'identity', position = 'dodge') +
    labs(x = 'Domain architecture similarity to yeast',
         y = 'Number of events') +
    scale_fill_discrete(guide = guide_legend(title = "Ortholog is essential in yeast?", 
                                             title.position = "top", 
    ...
    facet_wrap(~group) +
    coord_flip() +
    geom_text(aes(label = n, y = n), position = position_dodge(width = 0.9),
              size = 4, angle = 270, hjust = 0.5, vjust = -0.25)
}
