...
main <- function() {
  ...
  plot_number_orthologs(aligned_domain_archs)
...
plot_lost_dom_types <- function(lost_doms) {
  # ---------------------------------------------------------------------------
  # ---------------------------------------------------------------------------
  ggplot(filter(lost_doms, short_enough_for_domain_loss, !exclude_species), aes(x = species)) +
...
plot_number_orthologs <- function(aligned_domain_archs) {
  filter(aligned_domain_archs,
         !is.na(domain_arch) | !is.na(ref_domain_arch),
         !exclude_species) %>%
    group_by(species) %>%
    summarise(n = n()) %>%
